...
utils.removePoint = function(){
  console.log("remove one");
}
utils.addMenuToPoint = function(p){
  //var pointMenu = new BMap.ContextMenu();
  //pointMenu.addItem(new BMap.MenuItem('删除'), utils.removePoint)
  var point = new BMap.Point(p.lat,p.lng);
  point.addEventListener('click',function(){
    console.log("remove one");
  })
  //point.addContextMenu(pointMenu);
}

utils.addMenuToAllPoint = function(points){
  var size = points.length;
  for(var i=0; i< size; i++){
    utils.addMenuToPoint(points[i]);
  }
}
...
utils.ploygon = {
  ...
  addDbClick: function(ploygon){
    ploygon.addEventListener("dblclick",function(e){
      var points = ploygon.getPath();
      var ps = points.splice(1,1)
      ploygon.setPath(ps);
      console.log(e.point); 
    }) 
  }
